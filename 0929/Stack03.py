...
# "죠르디"는 게임의 재미를 높이기 위해 화면 구성과 규칙을
# 다음과 같이 게임 로직에 반영하려고 합니다.
...
def solution(board, moves):
    n = len(board)
    basket = []      # 바구니(스택)
    removed = 0      # 터진 인형 수 (2개씩 증가)

    for mv in moves:
        c = mv - 1   # moves는 1-based → 0-based로 변환
        # 위에서부터 내려오며 해당 열의 첫 인형을 찾음
        for r in range(n):
            if board[r][c] != 0:
                doll = board[r][c]
                board[r][c] = 0  # 집어서 비우기
                ...
                if basket and basket[-1] == doll:
                    ...
                    basket.append(doll)
                break            # 이 열에서는 하나만 집음(필수)
        # 인형이 없으면 아무 일도 일어나지 않음 → 그냥 다음 move로
