...
def solution(k, dungeons):
    ...
    used = [False] * n  # 각 던전을 이미 탐험했는지 표시
    best = 0  # 지금까지의 최대 탐험 수

    def dfs(cur_k, cleared):
        nonlocal best
        best = max(best, cleared)  # 현재까지 탐험한 개수로 최댓값 갱신

        for i in range(n):  # 아직 안 간 던전들을 하나씩 시도
            need, cost = dungeons[i]
            if not used[i] and cur_k >= need:  # (미사용) & (입장조건 충족)
                used[i] = True  # 선택 (방문 처리)
                dfs(cur_k - cost, cleared + 1)  # 소모하고 다음 단계로
                used[i] = False  # 되돌리기(백트래킹)

    dfs(k, 0)  # 시작: 현재 피로도 k, 탐험 0개
    return best
